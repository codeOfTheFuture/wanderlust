...
const Home: NextPage = () => {
	const { value, handleAddressChange, suggestions } = useAddressAutocomplete("");
	...
	const handleSearch = (e: FormEvent) => {
		...
		if (selectedSuggestion) dispatch(setSearchQuery(selectedSuggestion?.place_name as string));

		setZoomLevel(selectedSuggestion.place_type[0], selectedSuggestion.properties?.category);
	...
	return (
		<div className="w-full sm:w-5/6 lg:w-3/4 mx-auto">
			<Head>
				<title>Wanderlust - Visit National Parks, Historical Places, and Points of Interest</title>
			</Head>
			...
			<div className="relative flex flex-col justify-center md:justify-center items-center h-[70vh] z-10">
				<div className="flex flex-col items-start w-full justify-center gap-2 pl-5 md:pl-0">
					<h1 className="text-5xl md:text-6xl xl:text-8xl font-semibold text-light-text mt-52 sm:mt-44">
						Zion National Park
					</h1>
				...
				<form onSubmit={handleSearch} className="w-full absolute -bottom-[2.4rem] ">
					<SearchInput
						value={value}
...
export const getServerSideProps: GetServerSideProps = wrapper.getServerSideProps(
	store => async context => {
		...
		if (session && store.getState().user.user == null) {
			const user = (await db.collection("users").findOne({ email: session.user?.email })) as User;
		...
		const response = await db.collection("tours").find({}).limit(limit).toArray();
